...
const DEFAULT_NOTES={
 ...
 2:'판정 4단계의 차이를 짧은 예로 설명해요. “틀렸다”, “확인할 수 없다”, “맞지만 설명이 부족하다”는 서로 다른 판정이에요.',
 3:'혼자서 자료 카드를 대조하는 시간이에요(모둠 의논은 다음 단계). 다 쓰면 저장하기를 누르게 해요. 도움 요청이 오면 확인해요.',
 7:'먼저 모둠마다 기록자를 정하게 해요. 기록자만 입력할 수 있고 나머지는 의견을 말로 나눠요. 기록자가 저장하기를 눌러야 다른 모둠원 화면에 보여요.',
 4:'모둠 카드를 눌러 그 모둠의 발표 내용을 크게 보여 줘요(리모컨 →로 다음 모둠). 발표가 끝난 뒤 ‘모둠 판정 공개’를 켜면 학생 표가 채워져요.',
 5:'처음 판단과 비교해 무엇이 바뀌었는지 생각하게 해요. 자기평가를 체크하고 ‘제출하기’를 누르도록 안내해요.',
 6:'오늘의 한 문장을 함께 읽어요. 다음 차시엔 학생이 탐구 주제로 모아 온 자료의 출처를 오늘 방식대로 검증해 검증 노트로 써서 제출한다는 것을 안내하고 마무리해요.'
};
const STAGE_GUIDE={
 ...
 6:{todo:'오늘의 한 문장을 읽고 마무리해요',points:['AI의 답은 탐구의 출발점이지 인용할 결론이 아니다','출처를 추적하고, 조건을 묻고, 비교한 뒤에 쓴다','다음 시간: 내가 모아 온 자료도 오늘처럼 검증해 검증 노트 쓰기']}
};
